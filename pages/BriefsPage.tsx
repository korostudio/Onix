...
const BriefsPage: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<Brief['status'] | 'all'>('all');
  ...
  const { briefs, projects, clients, deleteBrief, shareDocument } = context;
  ...
  }, [briefs, projects, clients]);

  const filteredBriefs = useMemo(() => {
    return briefsWithDetails.filter(brief => {
      const searchTermMatch =
        ...
        brief.briefCode.toLowerCase().includes(searchTerm.toLowerCase());

      const statusMatch = statusFilter === 'all' || brief.status === statusFilter;
      ...
      const dateMatch = (!start || briefDate >= start) && (!end || briefDate <= end);

      return searchTermMatch && statusMatch && dateMatch;
    });
  }, [briefsWithDetails, searchTerm, statusFilter, startDate, endDate]);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => navigate('/briefs/new')} leftIcon={<PlusCircle size={18} />}>
          Crear Brief
        </Button>
      </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-neutral-500 dark:text-neutral-400 uppercase bg-neutral-50 dark:bg-neutral-900/50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 font-medium">Título del Brief</th>
            ...
            <tbody className="text-neutral-800 dark:text-neutral-200">
              {filteredBriefs.map((brief) => (
                <tr key={brief.id} className="border-b border-light-border dark:border-dark-border last:border-b-0 hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                  <td className="px-6 py-4 font-semibold font-mono text-xs">{brief.briefCode}</td>
                  <td className="px-6 py-4 font-semibold">
                    <div className="max-w-sm truncate" title={brief.title}>
                        {brief.title}
                    </div>
                  </td>
                  <td className="px-6 py-4">{brief.clientName}</td>
                  <td className="px-6 py-4">{brief.projectName}</td>
                  <td className="px-6 py-4 text-center">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full capitalize ${statusColors[brief.status]}`}>
                      {brief.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right space-x-3">
                    <button onClick={() => shareDocument('brief', brief.id)} className="text-neutral-500 hover:text-green-500 transition-colors" title="Compartir"><Share2 size={16}/></button>
                    <button onClick={() => navigate(`/briefs/${brief.id}/edit`)} className="text-neutral-500 hover:text-primary-600 dark:hover:text-primary-400 transition-colors" title="Editar"><Edit size={16}/></button>
                    <button onClick={() => deleteBrief(brief.id)} className="text-neutral-500 hover:text-red-500 transition-colors" title="Eliminar"><Trash2 size={16}/></button>
                  </td>
                </tr>
              ))}
            </tbody>
