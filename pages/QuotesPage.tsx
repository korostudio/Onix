...
const QuotesPage: React.FC = () => {
  ...
  const { quotes, clients, deleteQuote, shareDocument } = context;

  const quotesWithDetails = useMemo(() => {
  ...
  const filteredQuotes = useMemo(() => {
    return quotesWithDetails.filter(quote => {
  ...
  }, [quotesWithDetails, searchTerm, statusFilter, startDate, endDate]);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => navigate('/quotes/new')} leftIcon={<PlusCircle size={18} />}>
          Crear Cotización
        </Button>
      </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="text-neutral-800 dark:text-neutral-200">
              {filteredQuotes.map((quote) => (
                <tr key={quote.id} className="border-b border-light-border dark:border-dark-border last:border-b-0 hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                  <td className="px-6 py-4 font-semibold font-mono text-sm">{quote.quoteNumber}</td>
                  <td className="px-6 py-4 font-semibold whitespace-nowrap">{quote.clientName}</td>
                  <td className="px-6 py-4 font-mono">{formatCurrency(quote.amount)}</td>
                  <td className="px-6 py-4 text-center">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full capitalize ${statusColors[quote.status]}`}>
                      {quote.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right space-x-3">
                    <button onClick={() => shareDocument('quote', quote.id)} className="text-neutral-500 hover:text-green-500 transition-colors" title="Compartir"><Share2 size={16}/></button>
                    <button onClick={() => navigate(`/quotes/${quote.id}/edit`)} className="text-neutral-500 hover:text-primary-600 dark:hover:text-primary-400 transition-colors" title="Editar"><Edit size={16}/></button>
                    <button onClick={() => deleteQuote(quote.id)} className="text-neutral-500 hover:text-red-500 transition-colors" title="Eliminar"><Trash2 size={16}/></button>
                  </td>
                </tr>
              ))}
            </tbody>
